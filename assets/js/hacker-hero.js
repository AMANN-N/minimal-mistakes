...
async function initHackerHero() {
    ...
    try {
        ...
        // Check for debug/test mode via URL params
        const urlParams = new URLSearchParams(window.location.search);
        const testCompany = urlParams.get('test_company');

        let data;
        if (testCompany) {
            // Mock data for testing
            await typeLog(terminalOutput, "> DEBUG MODE: Simulating Corporate Network...", 10);
            data = {
                ip: '8.8.8.8',
                city: 'Mountain View',
        ...
        // Determine Greeting
        // Determine Greeting
        // Filter out common ISPs to avoid saying "Hello Comcast Cable Team!"
        const ispKeywords = ['isp', 'telecom', 'cable', 'internet', 'broadband', 'wireless', 'network', 'mobile', 'communications', 'online', 'service', 'net', 'wifi'];
        const orgLower = data.org ? data.org.toLowerCase() : '';
        const isISP = ispKeywords.some(keyword => orgLower.includes(keyword));
